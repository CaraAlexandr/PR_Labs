...
def export_data(data_list, export_format, filename):
    """Exports the scraped data to the specified format."""
    if export_format == "JSON":
        with open(f"{filename}.json", "w") as outfile:
            outfile.write(json.dumps(data_list, indent=2, ensure_ascii=False))
        ...
        export_to_csv(f"{filename}.csv", data_list)
